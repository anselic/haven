...
/// the target we compile to. haven currently emits code for the host, so the
...
fn leaf_class(ty: &Type) -> Class {
    match ty {
        ...
        Type::Projection { base, assoc, .. } => panic!(
            "associated type projection `{base}::{assoc}` reached ABI classification"),
