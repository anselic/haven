//! The `build` post-build script: `haven` compiles the named Haven program, runs
//! it once the artifact exists, and describes the build to it through the
//! environment.
//!
//! The point of the hook is that `haven` stays ignorant of what the script does -
...
/// is how the tests check `haven` runs it from the project root.
const PROBE: &str = "\
import std/env
import std/fs
...
proc main() i32 {
    show(\"HAVEN_PKG_NAME\");
    show(\"HAVEN_PKG_VERSION\");
    show(\"HAVEN_OUTPUT_KIND\");
    show(\"HAVEN_TARGET_OS\");

    let artifact: String = env::var(\"HAVEN_ARTIFACT\").unwrap();
...
fn runs_the_script_once_the_artifact_exists() {
    ...
    assert!(stdout.contains("ARTIFACT-EXISTS"),
        "HAVEN_ARTIFACT should name a file that exists by the time the script \
         runs:\n{stdout}");
    ...
    assert!(dir.path().join(".haven/build").is_dir(),
        "the compiled script belongs in .haven/build/");
}
...
fn the_environment_describes_the_build() {
    ...
    for expected in [
        "HAVEN_PKG_NAME=app",
        "HAVEN_PKG_VERSION=0.1.0",
        "HAVEN_OUTPUT_KIND=bin",
        &format!("HAVEN_TARGET_OS={}", std::env::consts::OS),
    ] {
        assert!(stdout.contains(expected), "missing `{expected}`:\n{stdout}");
    }
}

/// `HAVEN_OUTPUT_KIND` speaks the manifest's vocabulary, not the prose of the
...
fn output_kind_is_the_manifest_spelling() {
    ...
    assert!(res.status.success(), "build failed: {}", err(&res));
    let stdout = out(&res);
    assert!(stdout.contains("HAVEN_OUTPUT_KIND=cdylib"), "got:\n{stdout}");
    // and the artifact it is handed is the shared library, not some stale guess.
    assert!(stdout.contains("ARTIFACT-EXISTS"), "got:\n{stdout}");
}

/// A packaging step that fails has to fail the build - otherwise `haven build`
...
fn a_missing_script_is_reported() {
    ...
    assert!(e.contains("haven.toml"), "should point at where it was declared; got: {e}");
}
...
fn the_script_is_reused_until_it_changes() {
    ...
    let first = haven(dir.path(), &["build"]);
    ...
    let second = haven(dir.path(), &["build"]);
    ...
    let third = haven(dir.path(), &["build"]);
